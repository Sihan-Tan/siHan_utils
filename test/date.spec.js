...
describe("date.js", function () {
  ...
  describe("formatDate", function () {
    ...
    const CUR_DATE = `${curTime.getFullYear()}-${curTime.getMonth() + 1}-${
    ...
    it("测试返回yyyy-MM-dd", function () {
      ...
      expect(formatDate(fmt, new Date("2010-05-08").getTime())).to.equal(
        "2010-05-08"
      );
    });
    it("测试返回yyyy-MM-dd hh:mm:ss", function () {
      ...
      expect(formatDate(fmt, new Date("2018-08-08 06:30:30"))).to.equal(
        "2018-08-08 06:30:30"
      );
      expect(
        formatDate(fmt, new Date("2010-05-08 06:30:30").getTime())
      ).to.equal("2010-05-08 06:30:30");
    });
    it("测试返回yyyy-M-d h:m:s q S", function () {
      ...
      expect(formatDate(fmt, new Date("2018-08-08 06:30:30"))).to.include(
        "2018-8-8 6:30:30 3"
      );
      expect(
        formatDate(fmt, new Date("2010-05-08 06:30:30").getTime())
      ).to.include("2010-5-8 6:30:30 2");
    });
  ...
  describe("calcAge", function () {
    ...
    it("3个月", function () {
      const { age, unit } = calcAge(new Date("2020-8-26"));
    ...
    it("1岁", function () {
      const { age, unit } = calcAge("2019-11-23");
